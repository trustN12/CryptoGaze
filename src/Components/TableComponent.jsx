...
const SaveBtn = ({ data }) => {
  ...
  const handleClicksave = (e) => {
    ...
    setTimeout(() => {
      setIsClicked(false); // Reset animation after a short duration
    }, 700); // Adjust timing based on your preference
  };
...
const TableComponent = () => {
  let { data, curr } = useContext(CryptoContext);
  const glowStyle = {
  ...
  return (
    <>
      <motion.div
        className="flex flex-col mt-10 border-2  border-r-cyan border-t-cyan border-l-green border-b-green rounded overflow-hidden scrollbar-hidden"
        style={glowStyle}
              ...
              {data.map((url) => {
                return (
                  ...
                  >
                    ...
                    <td className="py-4">
                      {Number(url.market_cap_change_percentage_24h).toFixed(3)}{" "}
                      %
                    </td>
                    <td
                      className={
      ...
      <div className="flex  items-center justify-between mt-4 h-[2rem] capitalize">
